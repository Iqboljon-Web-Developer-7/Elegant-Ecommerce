          ...
          >
            {navLinks.map((link) => (
              <li key={link.label} className="list-none">
                <NavLink
                  to={link.to}
                  className={({ isActive }) => `text-sm hover:text-gray-200 transition-colors ${isActive && 'border-b'}`}
                >
                  {link.label}
                </NavLink>
              </li>
          ...
          <div className="flex flex-col md:flex-row gap-4 text-center text-xs">
            ...
            {legalLinks.map((link) => (
              <NavLink
                key={link.label}
                to={link.href}
                className={({ isActive }) => `text-gray-400 hover:text-gray-200 ${isActive && "text-white"}`}
              >
                {link.label}
              </NavLink>
            ))}
